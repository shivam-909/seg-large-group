...
function UserProfilePage() {
    useEffect(() => {
        async function getProfileData() {
            ...
            const userID = await axios.post('http://localhost:8000/api/echo', {}, {headers: {Authorization: `Bearer ${token}`}}).then(response => {
        ...
        getProfileData().catch(error => {console.log(error)});
    },[])
